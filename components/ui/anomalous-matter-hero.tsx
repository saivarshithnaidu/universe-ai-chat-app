...
function GenerativeArtScene() {
  ...
  useEffect(() => {
    ...
    // Icosahedron with shader material
    const geometry = new THREE.IcosahedronGeometry(1.2, 8);
    const material = new THREE.ShaderMaterial({
      uniforms: {
        time: { value: 0 },
        pointLightPos: { value: new THREE.Vector3(0, 0, 5) },
        color: { value: new THREE.Color(0x38bdf8) }, // sky-300 approx
      },
      vertexShader: `
        uniform float time;
...
export function AnomalousMatterHero({
  ...
  return (
    <section
      role="banner"
      className="relative w-full h-screen bg-[#09090b] text-white overflow-hidden"
    >
      <Suspense fallback={<div className="w-full h-full bg-[#09090b]" />}>
        <GenerativeArtScene />
      </Suspense>

      {/* Gradient overlay fading from bottom */}
      <div className="absolute inset-0 bg-gradient-to-t from-[#09090b] via-[#09090b]/60 to-transparent z-10 pointer-events-none" />
      ...
      <div className="relative z-20 flex flex-col items-center justify-end h-full pb-20 md:pb-32 text-center">
        {children ? (
          ...
          <div className="max-w-3xl px-6 animate-fade-in-long">
            {title && (
              <p className="text-xs font-mono tracking-[0.3em] text-sky-400/80 uppercase mb-4">
                {title}
              </p>
            )}
            {subtitle && (
            ...
            {description && (
              <p className="max-w-xl mx-auto text-base leading-relaxed text-neutral-400">
                {description}
              </p>
            )}
          </div>
